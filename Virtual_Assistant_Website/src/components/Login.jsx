...
const Login = () => {
  ...
  const handleOnClick = (labelRef) => {
    ...
    labelRef.current.style.transform = "translateX(-0.5vw) translateY(-3vh) scale(0.75)";
  };
  ...
  return (
    <>
      <div className='flex flex-col items-center justify-center bg-black h-screen -z-50 containerr'>
        ...
        <div id='boxe' className="box z-50 flex flex-col items-center justify-center border-4 rounded-3xl  sm:w-9/12 sm:h-96 sm:mt-12 sm:gap-16 md:w-3/5">
          <div className='flex flex-col'>
            <label htmlFor='name' ref={labelRef1} className='text-black'>Name</label>
            <div><input onChange={onChange} name="name" onClick={() => handleOnClick(labelRef1)} type="text" required="" className='bg-transparent text-black outline-none' /></div>
            <div className='bg-black  h-1' id='line'></div>
          </div>
          <div className='flex flex-col'>
            <label htmlFor='email' ref={labelRef2} className='text-black'>Email</label>
            <div><input onChange={onChange} name="email" onClick={() => handleOnClick(labelRef2)} type="email" required="" className='bg-transparent text-black outline-none' /></div>
            <div className='bg-black  h-1' id='line'></div>
          </div>
          <div className='flex flex-col'>
            <label htmlFor='phone' ref={labelRef3} className='text-black'>Phone</label>
            <div><input onChange={onChange} name="phone" onClick={() => handleOnClick(labelRef3)} type="phone" required="" className='bg-transparent text-black outline-none' /></div>
            <div className='bg-black  h-1' id='line'></div>
          </div>
          <div>
            <button onClick={handleSubmit} id='btn' className='w-36 h-12 bg-black text-white border-4 border-gray-500'>Send</button>
          </div>
        </div>
      </div>
    </>
